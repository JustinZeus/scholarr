...
class ScholarIngestionService:
    ...
    async def run_for_user(self, db_session: AsyncSession, *, user_id: int, trigger_type: RunTriggerType, request_delay_seconds: int, network_error_retries: int = 1, retry_backoff_seconds: float = 1.0, max_pages_per_scholar: int = 30, page_size: int = 100, scholar_profile_ids: set[int] | None = None, start_cstart_by_scholar_id: dict[int, int] | None = None, auto_queue_continuations: bool = True, queue_delay_seconds: int = 60, idempotency_key: str | None = None, alert_blocked_failure_threshold: int = 1, alert_network_failure_threshold: int = 2, alert_retry_scheduled_threshold: int = 3) -> RunExecutionSummary:
        ...
        paging_kwargs = self._paging_kwargs(request_delay_seconds=effective_request_delay_seconds, network_error_retries=network_error_retries, retry_backoff_seconds=retry_backoff_seconds, rate_limit_retries=settings.ingestion_rate_limit_retries, rate_limit_backoff_seconds=settings.ingestion_rate_limit_backoff_seconds, max_pages_per_scholar=max_pages_per_scholar, page_size=page_size)
        ...
        *,
        ...
        if state.parsed_page.publications:
            discovered_count = await self._upsert_profile_publications(
                db_session, run=run, scholar=scholar, publications=list(state.parsed_page.publications)
            )
        ...
        while state.parsed_page.has_show_more_button:
            ...
            # Immediately commit the new page's publications to the DB
            if next_parsed_page.publications:
                discovered_count = await self._upsert_profile_publications(
                    db_session, run=run, scholar=scholar, publications=list(next_parsed_page.publications)
                )
                state.discovered_publication_count += discovered_count
                await db_session.commit()
        ...
        for i in range(0, len(publications), batch_size):
            ...
            for p in batch:
                match = find_best_match(
                    ...
                    target_year=p.year,
                    target_authors=p.authors_text or scholar.name,
    ...
    async def _upsert_profile_publications(
        ...
        *,
        ...
        publications = await self._enrich_publications_with_openalex(scholar, publications)
        ...
        for candidate in publications:
            ...
            # Commit immediately to ensure the row becomes visible to the UI immediately
            await db_session.commit()
            
            # Emit Server-Sent Event for live UI progress updates
    ...
    async def _resolve_publication(
        ...
        if publication is None:
            ...
            await identifier_service.discover_and_sync_identifiers_for_publication(
                db_session,
                publication=created,
                scholar_label=candidate.authors_text or "",
            )
            return created
